...
async fn run() -> Result<()> {
    ...
    let mut exit_code = 0;
    tokio::select! {
        // handle termination signals (ctrl+c / SIGTERM) so the shutdown path
        // below runs. In particular `docker stop` and most process managers
        // send SIGTERM; without handling it the process is killed before the
        // feilian_v1 logout that frees the server-side terminal slot.
