...
# libwg/version.go (the wg-corplink version string) is generated by the
# Makefile from `git describe` and is gitignored, so it is absent from the
# build context / CI checkout. We build libwg directly (not via make) and
# .dockerignore drops .git, so generate it here — otherwise the c-archive
# build fails with "undefined: Version" in libwg/main.go.
ARG WG_VERSION=0.0.20220316-51-g0efa736
